...
- 前端不需要理解引擎内部实现，只维护 Workflow JSON 草稿。
...
编辑器在内存中维护一份 **Workflow JSON 草稿**：

- 尽量遵守 `engine_dsl.schema.json` 中 `$defs.Workflow` 的结构；
- 允许是“不完全合法”的中间状态（比如某个节点临时没有 `node_type`，某条边的 `target` 还没选好）。

后续所有 introspection / validate / save 接口，都直接以这份草稿 Workflow 作为请求体。

### 3. Introspection：变量 / 类型推导

#### 3.1 推导全局 State 字段 & 读写关系

- 接口：`POST /api/v1/workflows/schema`
- 请求体：当前 Workflow 草稿 JSON
- 返回：`WorkflowSchemaResponse`
...
- 在某些高级界面（例如“全局变量列表”）展示当前 Workflow 里有哪些字段；
- 作为变量搜索/自动补全的候选集合。

#### 3.2 每个节点可用变量视图

- 接口（全部节点）：`POST /api/v1/workflows/vars`  
  - 请求体：当前 Workflow 草稿 JSON  
  - 返回：`WorkflowVarsResponse`

- 接口（单个节点，可选）：`POST /api/v1/workflows/vars/node`  
  - 请求体示例：

    ```jsonc
    {
      "workflow": { /* Workflow 草稿 JSON */ },
...
`WorkflowVar.variable` 字段就是“在表达式/模板里要写的变量名/路径字符串”，前端可以把它当成**不透明的片段**，直接插入即可，不需要自己去推导它背后是如何从 State 计算出来的——后端保证它在当前 Workflow 上是合法的。

推荐的命名前缀（前缀层面会尽量保持稳定）：

- `state.*` —— Workflow 级 State 字段
...
- `workflow.*` —— Workflow 级输入或元信息（如果暴露）
  - 例：`workflow.input_as_text`（聊天类 Workflow 的输入文本）。

前端使用建议：

- 把 `variable` 当成表达式片段：
  - 在变量列表中展示时，可以按首段分组（例如 state/nodes/workflow），方便用户浏览；
...
当使用单节点接口（`/workflows/vars/node`）时，返回的只有当前正在编辑节点的
...
- 接口：`POST /api/v1/workflows/edges/inspect`
- 请求体：
...
- Workflow 自身结构是否完整：version/name/nodes/edges/start_node_id 等；
...
- 创建：
  ...
  - 请求体：最终确认后的 `Workflow` JSON
  - 返回：`WorkflowResponse`，包含 `id/created_at/updated_at` 等。

- 更新：
  - 接口：`PUT /api/v1/workflows/{id}`
  - 请求体：更新后的 `Workflow` JSON。

前端用途：

- 把当前画布对应的 Workflow 保存为一个版本化对象（类似 OpenAI Agent Builder 的“Publish”）。

### 6. 执行 Workflow

- 接口：`POST /api/v1/workflows/{id}/execute`  
  或：`POST /api/v1/workflows/{id}/execute/stream`（SSE）
...
- 前端只需要维护一份 Workflow 草稿 JSON，并在需要时把它发送给后端：
  - `/components`：初始化组件目录；
  - `/workflows/schema`：全局 State 视图；
  - `/workflows/vars`：节点级变量视图；
  - `/workflows/validate`：整体校验；
  - `/workflows`：保存 / 发布；
  - `/workflows/{id}/execute`：执行。
