	...
	return registry.ComponentMetadata{
		...
		Inputs: []registry.ParameterSchema{
			...
			},
			{
				...
				Required:    false,
				Description: "List of MCP tool configurations. Each config should have: transport (stdio/streamable_http/sse), server_url (for http/sse), command/args (for stdio), timeout, tool_filter",
			},
			{
