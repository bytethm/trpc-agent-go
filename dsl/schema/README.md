...
- Frontend editors (visual workflow builders)
...
- **`Workflow`**
  - The root type for a workflow DSL document.
  - Fields:
...
- **`Node`**
  - An executable node in the workflow.
  - Fields:
  ...
  - Note: `inputs` / `outputs` are *not* part of the workflow schema. Node I/O comes from component metadata (see below).
...
- **`StateVariable`**
  - Workflow‑level global variable declaration.
  - Fields:
...
The I/O schema for each component is not encoded inside the workflow; it is described by **component metadata** and exposed via the `/api/v1/components` API. The corresponding schema types are:

- **`ComponentMetadata`** (in `engine_dsl.schema.json` `$defs`)
...
2. **Edit and save workflows**
   - When saving, construct a JSON object that matches the `Workflow` definition in `engine_dsl.schema.json`.
...
3. **Introspection / variable suggestions**
   - The `/api/v1/workflows/schema` and `/api/v1/workflows/vars` endpoints provide:
     - Inferred state fields and usages (`FieldUsage`).
     - Per‑node variables (`WorkflowVars*`).
...
- **`builtin.start`**
  ...
  - `config_schema` (`ParameterSchema[]`): currently empty; may be extended in the future (for example to declare workflow‑level variables).
...
- **`builtin.set_state`**
  ...
  - `outputs`:
    - No explicit `outputs` entry; updates workflow‑level `state_variables` via assignments.
  - `config_schema`:
